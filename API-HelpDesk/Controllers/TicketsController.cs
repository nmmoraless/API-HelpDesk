...
namespace API_HelpDesk.Controllers
{
...

    {
        ...
        public string GuardarData([FromBody] NewTicket TicketNew)
        {
            //Instaciar conexión a base de datos
            TicketDAL ticketDAL = new TicketDAL();
            string resultado = ticketDAL.CrearNuevoTicket(TicketNew);
            return resultado;
        ...
        public IList<NewTicket> ActualizarData([FromBody] NewTicket TicketNew, int id)
        {
            int position;
        ...
        public IList<NewTicket> DeleteData(int id)
        {
            for (var i = 0; i < TicketList.Count; i++)
            {
                if (TicketList[i].Id == id)
